...
static int default_RNG(uint8_t *p_dest, unsigned p_size)
{
    ...
    (defined(__APPLE__) && defined(__MACH__)) || defined(uECC_POSIX)) && !defined(_KERNEL)
...
void uECC_set_rng(uECC_RNG_Function p_rng)
{
    ...
    #if (defined(uECC_ASM) && (uECC_PLATFORM == uECC_arm || uECC_PLATFORM == uECC_arm_thumb || uECC_PLATFORM == uECC_arm_thumb2))
        #include "asm_arm.inc"
    #endif
#endif
